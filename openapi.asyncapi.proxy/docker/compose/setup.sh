...
if [[ -z $(docker compose -p $NAMESPACE ps -q zilla) ]]; then
  ...
  if [[ $INIT_KAFKA == true ]]; then
    ...
    /opt/bitnami/kafka/bin/kafka-topics.sh --bootstrap-server $KAFKA_BOOTSTRAP_SERVER --create --if-not-exists --topic requests
    /opt/bitnami/kafka/bin/kafka-topics.sh --bootstrap-server $KAFKA_BOOTSTRAP_SERVER --create --if-not-exists --topic responses
    "
  fi
